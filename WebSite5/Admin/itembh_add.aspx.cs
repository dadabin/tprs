...
public partial class Admin_itembh_add : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), "msg", "<script>alert('占用土地输入的数字不能长于9位！')</script>");
            return;
        }
        ...
        if (TextBox15.Text != null && TextBox15.Text.Trim() != "")
        {
            ...
            {
                Page.ClientScript.RegisterStartupScript(this.GetType(), "msg", "<script>alert('单价输入格式有误！')</script>");
                return;
            }
    ...
    {
        string inputName = this.TextBox2.Text.ToString().Trim();
        ItemBll bll = new ItemBll();
        if (bll.isExitProject(2, inputName))
        {
