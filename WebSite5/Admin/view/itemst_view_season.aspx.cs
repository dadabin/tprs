...
public partial class Admin_view_itemst_view_season : System.Web.UI.Page
{
    ...
    private void bind()
    {
        if (Request.QueryString["id"] != null && Request.QueryString["id"].ToString() != "")
        {
            ...
            DataTable dt = bll.selectReportById (2, int.Parse(Request.QueryString["id"]));
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                Label4.Text = dt.Rows[0]["PROGRESSCATEGORY"].ToString();//建设年限
                ...
                Label16.Text = dt.Rows[0]["MUNICIPALMONEY"].ToString();//市级资金
                Label17.Text = dt.Rows[0]["DISTRICTMONEY"].ToString();//区县资金
                Label18.Text = dt.Rows[0]["BANKLOAN"].ToString();//银行贷款
                ...
                Label24.Text = dt.Rows[0]["PROGRESSCATEGORY"].ToString();//四个一批范畴
                Label25.Text = dt.Rows[0]["GETLANDTARGETS"].ToString();//已取得土地指标
                Label26.Text = dt.Rows[0]["PLANLANDTARGETS"].ToString();//拟申请土地指标
                Label27.Text = dt.Rows[0]["GOVERNMENTPROJE"].ToString().Trim();//是否政府投资项目
